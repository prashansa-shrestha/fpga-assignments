...
1. **Control Unit ([control_unit.sv](./control_unit.sv)):** 
...
2. **Register Bank ([register_bank.sv](./register_bank.sv)):**
...
5. **Top-Level CPU ([processor.sv](./processor.sv)):**
...
| **ARITH (ALU Ops)** | `1_oooo_rrr` | `R0 <= R0 op R_r` (where `oooo` specifies ALU opcode) | 1 |
...
- `0100` : **DEC** (`R0 <= R0 - 1`)
- `0101` : **COMP** (`R0 <= ~R0`)
- `0110` : **LSHIFT** (`R0 <= R0 << R_r[2:0]`)
- `0111` : **RSHIFT** (`R0 <= R0 >> R_r[2:0]`)
...
The testbench [processor_tb.sv](./processor_tb.sv) runs a test program validating the interaction of all modules:
